...
# we allow sane creative ranges only. temperature/tint are -1..1 sliders
# (cool<->warm, green<->magenta); sharpen and vignette are 0..1 amounts.
ADJUST_RANGES = {
...
_EQ_KEYS = ("brightness", "saturation", "contrast")


def adjust_chain(adjust: dict | None) -> str:
    """FFmpeg fragment for the manual adjustments, or '' when everything is
    at its neutral default. Values are clamped, and only numbers survive —
    nothing user-provided is interpolated as syntax."""
    if not adjust:
        return ""
    vals: dict[str, float] = {}
    for key, (lo, hi, default) in ADJUST_RANGES.items():
        ...
        if abs(value - default) > 0.005:
            vals[key] = value
    parts: list[str] = []
    eq = [f"{k}={vals[k]:.3f}" for k in _EQ_KEYS if k in vals]
    if eq:
        parts.append("eq=" + ":".join(eq))
    cb: list[str] = []
    if "temperature" in vals:  # + warm (red up / blue down), - cool
        cb.append(f"rm={0.15 * vals['temperature']:.3f}")
        cb.append(f"bm={-0.15 * vals['temperature']:.3f}")
    if "tint" in vals:  # + magenta (green down), - green
        cb.append(f"gm={-0.12 * vals['tint']:.3f}")
    if cb:
        parts.append("colorbalance=" + ":".join(cb))
    if "sharpen" in vals:
        parts.append(f"unsharp=5:5:{1.5 * vals['sharpen']:.3f}")
    if "vignette" in vals:  # angle maps 0..1 to subtle..strong (max ~PI/2)
        parts.append(f"vignette=angle={0.3 + 1.2 * vals['vignette']:.3f}")
    return ",".join(parts)
